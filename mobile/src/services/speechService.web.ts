...
export async function startSpeechRecognition(language = 'en-IN'): Promise<SpeechResult> {
  ...
  return {
    ...
    note: 'Guided dictation is active in browser preview. Use the native mobile build for live microphone speech.'
  };
}
...
export function getSpeechSupportMessage() {
  return 'Browser preview uses guided dictation. Use the custom native build for real microphone speech recognition.';
}
